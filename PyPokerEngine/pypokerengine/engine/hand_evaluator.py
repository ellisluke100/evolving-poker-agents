...
class HandEvaluator:
  ...
  def eval_hand(self, hole, community):
    ...
    # Hole = list e.g. ST, H6
    # Swap them around
    # Lowercase the card suit
    # Make holes / community
    # Run evaluator
    # Return the value
    ...
    for h in hole:
      suit = (h.SUIT_MAP[h.suit]).lower()
      rank = h.RANK_MAP[h.rank]
      hand.append(Card.new(rank+suit))

    for c in community:
      suit = (c.SUIT_MAP[c.suit]).lower()
      rank = c.RANK_MAP[c.rank]
      board.append(Card.new(rank+suit))
